...
export default class FieldColorSource extends Component {
  constructor(props) {
    super(props);
  }
      ...
      <Fragment>
        ...
        <ul className={classnames({ 'acms-admin-table-admin-edit': acmscss })} ref={(table => this.table = table)} >
          {customfield.map((item, index) => {
            if (item.type === 'text') {
              return (<li key={index} className="colorList">
                  <div>
              ...
              return (
                <li key={index} className="colorList">
                  <div>
